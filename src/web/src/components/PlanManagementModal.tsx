...
import { X, Calendar, Clock, AlertTriangle, Check, Star, Shield } from 'lucide-react';
...
export default function PlanManagementModal({
  ...
  const isTrialActive = user.trial_start_date && user.plan === 'pro' && user.plan_expires_at;
  ...
  const handleUpdatePlan = async () => {
    ...
    try {
      ...
      const expiresAt = expiryDate ? new Date(expiryDate).toISOString() : undefined;
      await onUpdatePlan(user.id, selectedPlan, expiresAt);
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Errore nell\'aggiornamento del piano');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStartTrial = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Errore nell\'avvio del trial');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6 border-b border-border">
              <h3 className="text-lg font-semibold text-foreground mb-4">Piano Attuale</h3>
              
              <div className="flex items-center space-x-4 p-4 bg-surface rounded-xl">
                <div className={`w-12 h-12 bg-gradient-to-br ${planColors[user.plan]} rounded-full flex items-center justify-center`}>
                  {(() => {
                ...
                <div className="flex-1">
                  ...
                  {user.plan_expires_at && (
                    <div className="flex items-center space-x-1 mt-2 text-sm text-foreground-secondary">
                      <Calendar size={14} />
                      <span>
                        Scade il {formatDate(user.plan_expires_at)}
                      </span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-blue-300 mt-1">
                        Questo utente può iniziare un trial Pro gratuito di 7 giorni.
                      </p>
                      <button
            ...
            {/* Plan Selection */}
            <div className="p-6">
              <h3 className="text-lg font-semibold text-foreground mb-4">Cambia Piano</h3>
              ...
              <div className="space-y-3 mb-6">
                {Object.entries(PLAN_CONFIGS).map(([planType, config]) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-10 h-10 bg-gradient-to-br ${planColors[planType as PlanType]} rounded-full flex items-center justify-center`}>
              ...
              <div className="flex items-center justify-end space-x-3">
                ...
                <button
                  ...
                  disabled={loading || (selectedPlan === user.plan && !expiryDate)}
